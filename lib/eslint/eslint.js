 ...
 * @property {boolean} [applySuppressions] If true, apply suppressions automatically. Defaults to false.
 * @property {string} [suppressionsLocation] Path to suppressions file. Relative to cwd. Defaults to eslint-suppressions.json in cwd.
 */
...
class ESLint {
	...
	constructor(options = {}) {
		...
		if (processedOptions.applySuppressions) {
			const suppressionsFilePath = getCacheFile(
				processedOptions.suppressionsLocation || "eslint-suppressions.json",
				processedOptions.cwd,
	...
	async lintFiles(patterns) {
		...
		let finalResults = results.filter(result => !!result);
		...
		if (suppressionsService) {
			...
			finalResults = suppressedResults;
		}
	...
	async lintText(code, options = {}) {
		...
		const { linter, options: eslintOptions, suppressionsService } = privateMembers.get(this);
		const {
			allowInlineConfig,
			cwd,
			fix,
		...
		if (suppressionsService && filePath) {
			const { results: suppressedResults } = suppressionsService.applySuppressions(
				results,
				await suppressionsService.load(),
			);
