...
describe("ESLint", () => {
	...
	describe("hasFlag", () => {
		...
		it("should not throw when 'applySuppressions' is a boolean", () => {
			assert.doesNotThrow(() => {
				new ESLint({ applySuppressions: true }); // eslint-disable-line no-new -- Check for argument side effects
			});
			assert.doesNotThrow(() => {
				new ESLint({ applySuppressions: false }); // eslint-disable-line no-new -- Check for argument side effects
			});
		});
		...
		it("should not throw when 'suppressionsLocation' is a non-empty string or null", () => {
			assert.doesNotThrow(() => {
				new ESLint({ suppressionsLocation: "my-suppressions.json" }); // eslint-disable-line no-new -- Check for argument side effects
			});
			assert.doesNotThrow(() => {
				new ESLint({ suppressionsLocation: null }); // eslint-disable-line no-new -- Check for argument side effects
			});
		});
	...
	describe("lintText()", () => {
		...
		it("should apply suppressions when 'applySuppressions' is true and 'filePath' is provided", async () => {
			...
			const noUndefMessages = results[0].messages.filter(m => m.ruleId === "no-undef");

			assert.strictEqual(noUndefMessages.length, 0, "no-undef errors should be suppressed");

			// Check that suppressed messages are in suppressedMessages
			const suppressedNoUndef = results[0].suppressedMessages.filter(m => m.ruleId === "no-undef");

			assert.strictEqual(suppressedNoUndef.length, 3, "no-undef errors should appear in suppressedMessages");
		});
		...
		it("should not apply suppressions when 'applySuppressions' is false (default)", async () => {
			...
			assert.strictEqual(noUndefMessages.length, 3, "no-undef errors should not be suppressed");
		});
		...
		it("should not apply suppressions when 'filePath' is omitted", async () => {
			...
			assert.strictEqual(noUndefMessages.length, 3, "no-undef errors should not be suppressed without filePath");
		});
	});
	...
	describe("lintFiles()", () => {
		...
		it("should apply suppressions to lintFiles results when 'applySuppressions' is true", async () => {
			...
			const noUndefMessages = results[0].messages.filter(m => m.ruleId === "no-undef");
			const noSparseMessages = results[0].messages.filter(m => m.ruleId === "no-sparse-arrays");

			assert.strictEqual(noUndefMessages.length, 0, "no-undef errors should be suppressed");
			assert.strictEqual(noSparseMessages.length, 0, "no-sparse-arrays errors should be suppressed");

			// Suppressed messages should be in suppressedMessages
			const suppressedNoUndef = results[0].suppressedMessages.filter(m => m.ruleId === "no-undef");
			const suppressedNoSparse = results[0].suppressedMessages.filter(m => m.ruleId === "no-sparse-arrays");

			assert.strictEqual(suppressedNoUndef.length, 3, "no-undef errors should appear in suppressedMessages");
			assert.strictEqual(suppressedNoSparse.length, 2, "no-sparse-arrays errors should appear in suppressedMessages");
		});
		...
		it("should use default 'eslint-suppressions.json' when 'suppressionsLocation' is omitted", async () => {
			...
			assert.strictEqual(noUndefMessages.length, 0, "should use default suppressions file");
		});
		...
		it("should use custom suppressions file when 'suppressionsLocation' is provided", async () => {
			...
			assert.strictEqual(noUndefMessages.length, 3, "should not suppress errors when suppressions file doesn't match");
		});
		...
		it("should handle missing suppressions file gracefully when 'applySuppressions' is true", async () => {
			...
			assert.strictEqual(noUndefMessages.length, 3, "should not crash and should report all errors");
		});
	});
